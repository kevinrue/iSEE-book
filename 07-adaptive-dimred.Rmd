...
including interface elements and observers to perform multiple selections and respond to aesthetic parameters.
We also add a slot specifying the type of dimensionality reduction result and the number of highly variable genes to use. 
...
We would also like to add interface elements to change the result type and the number of genes.
...
We theb `paste0` the name of our panel to the name of any parameter to ensure that the ID is unique to this instance of our panel;
    ...
    function(x, se, input, session, pObjects, rObjects) 
{
    ...
    observeEvent(input[[type_field]], {
        ...
        .refreshPanelOutput(plot_name, pObjects, rObjects, clear=TRUE)
    })
...
By marking them as such, we instruct `iSEE()` to destroy existing brushes and lassos when these parameters are changed,
as brushes/lassos made on the previous plot do not make sense when the coordinates are recomputed.

## Making the plot

When working with a `ColumnDotPlot` subclass, the easiest way to change plotting content to override the `.getCommandsDataXY` method.
...
```{r}
setMethod(".getCommandsDataXY", "DynRedDimPlot", function(x, envir) {
...
If `col_selected` does exist, it contains a list of character vectors specifying the active and saved multiple selections that are being transmitted.
...
drdp <- new("DynRedDimPlot")
drdp[["SelectColSource"]] <- "RedDimPlot1"
```
