...
setValidity2("GeneAnnoTable", function(object) {
    ...
    if (!isSingleString(type <- object[["IDType"]]) || !type %in% allowable) {
        msg <- c(msg, sprintf("'IDType' should be 'ENSEMBL', 'SYMBOL' or 'ENTREZID'"))
    }
...
```{r}
setMethod("initialize", "GeneAnnoTable", function(.Object, 
    IDColumn=NULL, Organism="org.Mm.eg.db", IDType="SYMBOL", ...) 
{
...
For convenience, we will put these elements in the "Data parameters" box via the `.defineDataInterface()` generic,
even though they more accurately belong in a separate "Annotation parameters" box.

```{r}
setMethod(".defineDataInterface", "GeneAnnoTable", function(x, se, select_info) {
    panel_name <- .getEncodedName(x)
    list(
        callNextMethod(),
    ...
    function(x, se, input, session, pObjects, rObjects)
{
    ...
    # TODO: replace with an in-built event hook for single selections?
    anno_name <- paste0(plot_name, "_anno_bump")
    rObjects[[anno_name]] <- 1L
    observeEvent(input[[paste0(plot_name, "_rows_selected")]], {
        rObjects[[anno_name]] <- isolate(rObjects[[anno_name]]) + 1L 
    })
})
```
...
setMethod(".renderOutput", "GeneAnnoTable", function(x, se, ..., output, pObjects, rObjects) {
    ...
    output[[paste0(panel_name, "_annotation")]] <- renderUI({
        force(rObjects[[paste0(panel_name, "_anno_bump")]]) 
