...
\item
  \texttt{Panel} \O
  - \texttt{DotPlot} \O
   - \texttt{ColumnDotPlot} \O
   - \texttt{ReducedDimensionPlot} - \texttt{ColumnDataPlot} -
  \texttt{FeatureAssayPlot} - \texttt{RowDotPlot} \O
   - \texttt{RowDataPlot} - \texttt{SampleAssayPlot}
...
Some of those classes are ``virtual'' (indicated by \O), meaning that
they cannot be directly instanciated as panels in the GUI. Instead,
virtual panel classes define families of panels that share groups of
properties. As such, virtual classes are meant to be used as the parent
of concrete classes that share the associated properties.
