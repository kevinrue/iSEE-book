...
Refer to \texttt{help("Panel-class")} for more information about the slots and methods provided by this core class.
...
The \texttt{DotPlot} class introduces parameters specific to panels where the output is a \texttt{ggplot} object and each row in the data-frame is represented as a point in a plot.

The \texttt{Table} class introduces parameters specific to panels where the main output is a data-frame directly displayed as a table in the GUI.

In addition, the \texttt{HeatMapPlot} class defines a special concrete panel class that directly extends the \texttt{Panel} class, as it introduces a set of parameters distinct from both the \texttt{DotPlot} and \texttt{Table} panel families.
...
It allows the app to efficiently compute a single time common information that only depends on the input \texttt{se} object, and may be frequently reused during the runtime of an app.
...
Each panel type that derives from the virtual class \texttt{DotPlot} must define - or inherits - the \texttt{.generateDotPlot} method.

This function is called within \texttt{.renderOutput}, which is triggered by observers when the value of the input widgets are changed by users, or when a new panel is added to the GUI.

The \texttt{.generateDotPlot} function has access to the parameters for a given panel instance, and uses information available in the panel evaluation environment to generate and evaluate the plotting commands that ultimately produce the \texttt{ggplot} object to display in the panel.

Refer to the \textbf{``Generating the ggplot object''} section of \texttt{help(".generateDotPlot")} for more information.
...
Then, developing a new panel type starts with the creation of a new class that inherits from the \texttt{Panel} class.

While it is possible to create a new panel class that directly inherits from the top-most virtual \texttt{Panel} class, this is the most advanced use case that we will describe in a later section.
...
For instance, to create a panel that inherits all the functionality of the \emph{Reduced dimension plot} panel type, we simply define a new class that extends the \texttt{RedDimPlot} class.
We call that new class \texttt{RedDimHexPlot}.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{setClass}\NormalTok{(}\StringTok{"RedDimHexPlot"}\NormalTok{, }\DataTypeTok{contains=}\StringTok{"RedDimPlot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
To facilitate this, new panels should provide a constructor function - named identically to the class - to accept arbitrary arguments controlling the initialization of new panel instances created by the \texttt{new} function.

Here, we define a simple constructor function that passes all incoming arguments to the \texttt{new} function \emph{as is}.
...
However, that would not be very exciting as instances of this new panel class would behave exactly like the those of the \texttt{RedDimPlot} class.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{RedDimHexPlot1 <-}\StringTok{ }\KeywordTok{RedDimHexPlot}\NormalTok{()}
\NormalTok{RedDimPlot1 <-}\StringTok{ }\KeywordTok{RedDimPlot}\NormalTok{()}
\NormalTok{initial <-}\StringTok{ }\KeywordTok{list}\NormalTok{(RedDimHexPlot1, RedDimPlot1)}
\NormalTok{app <-}\StringTok{ }\KeywordTok{iSEE}\NormalTok{(sce, }\DataTypeTok{initial =}\NormalTok{ initial)}
\end{Highlighting}
\end{Shaded}
...
The name of each panel displayed in the GUI is defined by the \texttt{.fullName} method.
...
With that, running \texttt{app} again now highlights how panels of the new class now display a different title from the parent class.
...
If a new \texttt{DotPlot} panel class is meant to process data in the same way as its parent panel, only to display in a different way, it is then possible to overwrite only the \texttt{.generateDotPlot} method.
Meanwhile, the data preprocessing will be implicitly handled by the other API methods inherited from the parent class.

Importantly, the \texttt{.generateDotPlot} method requires two key arguments: \texttt{labels} provides the plot labels for each of the aesthetics in the plot data, and \texttt{envir} provides the environment in which the plotting commands are to be evaluated to produce the \texttt{ggplot} object.

In particular, \texttt{.generateDotPlot} can rely on certain environment variables promised by the contract.
Above all, those include \texttt{plot.data}, the data-frame that contains one row per data point to display.
...
We invite readers to refer to the \textbf{``Generating the ggplot object''} section of \texttt{help(".generateDotPlot")} for more information.

As an example, we overwrite the \texttt{.generateDotPlot} function for the new \texttt{RedDimHexPlot} class to simply show the number of data point in the plotting area as a heatmap dividing the plane into regular hexagons.
Notably, the function can immediately rely on the \texttt{plot.data} data-frame that is computed by methods defined for the parent class \texttt{RedDimPlot}.
We also use the precomputed aesthetic \texttt{labels} associated with each column of \texttt{plot.data}, while setting a fixed \texttt{"Count"} label for the \texttt{fill} aesthetic associated with the count of observation in each hexagonal bin.
...
\KeywordTok{setMethod}\NormalTok{(}\StringTok{".generateDotPlot"}\NormalTok{, }\StringTok{"RedDimHexPlot"}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x, labels, envir) \{}
    \KeywordTok{print}\NormalTok{(labels)}
...
Running \texttt{app} again highlights how the \texttt{RedDimHexPlot} panel fills each hexagonal bin with a color indicating the number of data points present in the corresponding area in the \texttt{RedDimPlot} panel.
...
Brushing at any location in \texttt{RedDimPlot1} will then trigger dynamically recompution of results in our \texttt{DynRedDimPlot}.
...
We do not ``un-hide'' \texttt{SelectColType} and \texttt{SelectColSaved} here;
...
\KeywordTok{setMethod}\NormalTok{(}\StringTok{".hideInterface"}\NormalTok{, }\StringTok{"DGETable"}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x, field) \{}
    \ControlFlowTok{if}\NormalTok{ (field }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"SelectRowSource"}\NormalTok{, }\StringTok{"SelectRowType"}\NormalTok{, }\StringTok{"SelectRowSaved"}\NormalTok{)) \{}
        \OtherTok{TRUE}
\NormalTok{    \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (field }\OperatorTok{%in%}\StringTok{ "SelectColSource"}\NormalTok{) \{}
        \OtherTok{FALSE}
\NormalTok{    \} }\ControlFlowTok{else}\NormalTok{ \{}
        \KeywordTok{callNextMethod}\NormalTok{()}
\NormalTok{    \}}
...
In such cases, we would not need to hide \texttt{SelectRowSource}, though we will leave that as an exercise for the curious.
...
\NormalTok{dget <-}\StringTok{ }\KeywordTok{new}\NormalTok{(}\StringTok{"DGETable"}\NormalTok{, }\DataTypeTok{SelectColSource=}\StringTok{"RedDimPlot1"}\NormalTok{, }\DataTypeTok{PanelWidth=}\NormalTok{8L)}
...
First, we define the basics of our new \texttt{Panel} class.
Our new class will be showing the gene-level metadata, so we inherit from the \texttt{RowStatTable} class that does exactly this.
...
\begin{verbatim}
## Class "GeneAnnoTable" [in ".GlobalEnv"]
## 
## Slots:
##                                                                
## Name:            IDColumn             IDType           Organism
## Class:  character_OR_NULL          character          character
##                                                                
## Name:         AnnoBoxOpen           Selected             Search
...
## 
## Extends: 
## Class "RowStatTable", directly
## Class "RowTable", by class "RowStatTable", distance 2
## Class "Table", by class "RowStatTable", distance 3
## Class "Panel", by class "RowStatTable", distance 4
\end{verbatim}
...
\NormalTok{    panel_name <-}\StringTok{ }\KeywordTok{.getEncodedName}\NormalTok{(x)}
    \KeywordTok{tagList}\NormalTok{(}
        \KeywordTok{callNextMethod}\NormalTok{(), }\CommentTok{# Re-using RowStatTable's definition.}
...
\NormalTok{    panel_name <-}\StringTok{ }\KeywordTok{.getEncodedName}\NormalTok{(x)}
    \KeywordTok{c}\NormalTok{(}
        \KeywordTok{list}\NormalTok{(}
            \CommentTok{# }\AlertTok{TODO}\CommentTok{: export this.}
\NormalTok{            iSEE}\OperatorTok{:::}\KeywordTok{collapseBox}\NormalTok{(}
...
We specialize \texttt{.createObservers} to define some observers to respond to changes in our new interface elements.
Note the use of \texttt{callNextMethod()} to ensure that observers of the parent class are also created.
We also create a reactive variable to indicate whether the selection has updated.
...
\NormalTok{    plot_name <-}\StringTok{ }\KeywordTok{.getEncodedName}\NormalTok{(x)}
    ...
    \CommentTok{# }\AlertTok{TODO}\CommentTok{: replace with an in-built event hook for single selections?}
\NormalTok{    anno_name <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(plot_name, }\StringTok{"_anno_bump"}\NormalTok{)}
\NormalTok{    rObjects[[anno_name]] <-}\StringTok{ }\NormalTok{1L}
    \KeywordTok{observeEvent}\NormalTok{(input[[}\KeywordTok{paste0}\NormalTok{(plot_name, }\StringTok{"_rows_selected"}\NormalTok{)]], \{}
\NormalTok{        rObjects[[anno_name]] <-}\StringTok{ }\KeywordTok{isolate}\NormalTok{(rObjects[[anno_name]]) }\OperatorTok{+}\StringTok{ }\NormalTok{1L }
\NormalTok{    \})}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
This updates in response to the reactive variable that we defined earlier.
...
\KeywordTok{setMethod}\NormalTok{(}\StringTok{".renderOutput"}\NormalTok{, }\StringTok{"GeneAnnoTable"}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x, se, ..., output, pObjects, rObjects) \{}
    \KeywordTok{callNextMethod}\NormalTok{() }\CommentTok{# Re-using RowStatTable's output rendering.}
...
\NormalTok{    output[[}\KeywordTok{paste0}\NormalTok{(panel_name, }\StringTok{"_annotation"}\NormalTok{)]] <-}\StringTok{ }\KeywordTok{renderUI}\NormalTok{(\{}
        \KeywordTok{force}\NormalTok{(rObjects[[}\KeywordTok{paste0}\NormalTok{(panel_name, }\StringTok{"_anno_bump"}\NormalTok{)]]) }
