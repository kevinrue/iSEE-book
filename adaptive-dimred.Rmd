...
```{r, echo=FALSE}
library(iSEE)
setClass("DynReducedDimensionPlot", contains="ColumnDotPlot",
...
```{r}
library(shiny)
setMethod(".defineDataInterface", "DynReducedDimensionPlot", function(x, se, select_info) {
...
We call `.getEncodedName()` to obtain a unique name for the current instance of our panel, e.g., `DynReducedDimensionPlot1`.
...
otherwise, multiple `DynReducedDimensionPlot`s would override each other.
...
Brushing at any location in `ReducedDimensionPlot1` will then trigger dynamically recompution of results in our `DynReducedDimensionPlot`.
