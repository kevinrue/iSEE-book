...
This considers the typical use case where `RowTable`s respond to a selection of rows,
rather than a selection of columns as in our `DGETable`.
...
We do so by specializing the `.hideInterface()` method, 
which returns `TRUE` to indicate that a particular interface element should be hidden.
We do not "un-hide" `SelectColType` and `SelectColSaved` here;
...
setMethod(".hideInterface", "DGETable", function(x, field) {
    if (field %in% c("SelectRowSource", "SelectRowType", "SelectRowSaved")) {
        TRUE
    } else if (field %in% "SelectColSource") {
        FALSE
    } else {
        callNextMethod()
    }
...
In such cases, we would not need to hide `SelectRowSource`, though we will leave that as an exercise for the curious.
...
dget <- new("DGETable", SelectColSource="RedDimPlot1", PanelWidth=8L)
