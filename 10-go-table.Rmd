...
This is a fairly involved example of creating a `Panel` subclass as we cannot easily inherit from an existing subclass;
...
Here, we use the `r BiocStyle::Biocpkg("GO.db")` package to create a table of GO terms and their definitions.
...
setMethod(".generateOutput", "GOTable", function(x, se, ..., all_memory, all_contents) {
    ...
    list(commands=list(commands), contents=envir$tab)
})
```
...
This also involves creating an observer to respond to the selected `DataTable` row.
    ...
    function(x, se, input, session, pObjects, rObjects)
{
    ...
    observeEvent(input[[select_field]], {
        ...
        # TODO: expose the API for this.
        iSEE:::.safe_reactive_bump(rObjects, multi_name)
        iSEE:::.mark_panel_as_modified(panel_name,
            c(iSEE:::.panelNorender, iSEE:::.panelReactivated),
            rObjects)
    }, ignoreNULL=FALSE)
...
In this case, we replace the selected row with an empty string to indicate that no selection has been made,
...
rst <- RowStatTable(SelectRowSource="GOTable1")
